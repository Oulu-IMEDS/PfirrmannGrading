...
def split_train_test(cfg, df):
    ...
    multimodal_df = df.iloc[train_indices]
    multimodal_df = multimodal_df.reset_index()
    test_df = df.iloc[test_indices]
    test_df = test_df.reset_index()
    return multimodal_df, test_df
...
def save_cm_kfold(cfg, fold, y_true, y_pred, tta, logger):
    ...
    if cfg.mode.multimodal.target_column == 'pfirrmann_grade':
        classes = ['2', '3', '4', '5']
    elif cfg.mode.multimodal.target_column == 'bmi_category':
        classes = ['Underweight', 'Normal', 'Overweight', 'Obese']
    elif cfg.mode.multimodal.target_column == 'sex':
        classes = ['Male', 'Female']
    elif cfg.mode.multimodal.target_column == 'diabetes':
...
def save_mis_classifications(cfg, dir_names, y_true, y_pred):
    ...
    odf = pd.read_csv(PurePath.joinpath(Path(cfg.mode.multimodal.dir, cfg.mode.multimodal.metadata)))
    for idx, (e1, e2) in enumerate(zip(y_true, y_pred)):
        if cfg.mode.multimodal.target_column == 'pfirrmann_grade':
            ...
            odf.loc[odf['dir_name'] == dir_names[idx], 'pred'] = e2
            if e1 != e2:
                mc.append([dir_names[idx], e1, e2])
    ...
    for i in range(16):
        plt.subplot(4, 4, i + 1)
        image_list = sorted(list(Path(cfg.mode.multimodal.dir, df.iloc[i]['dir_name']).glob("*.png")))
...
def score_avg_multi_modal_score(cfg, test_df, logger):
    logger.info(f"Test Set:{test_df.shape}")
    test_ds = MultiModalLoader(cfg, test_df, 'test')
    ...
    checkpoints = list(Path(PurePath.joinpath(Path.cwd(), "results", "models")).glob("SpineMMCls_*"))
    # checkpoints = list(Path("/home/nash/PycharmProjects/PfirrmannGrading/outputs/2022-06-22/16-53-25/results/models
    # ").glob("SpineMMCls_*"))
...
def score_avg_multi_modal_score_tta(cfg, test_df, logger):
    logger.info(f"Test Set:{test_df.shape}")
    test_ds = MultiModalLoader(cfg, test_df, 'train')
    test_loader = DataLoader(dataset=test_ds, batch_size=cfg.training.testing.batch_size,
                             num_workers=cfg.training.dataloader.num_workers)

    checkpoints = list(Path(PurePath.joinpath(Path.cwd(), "results", "models")).glob("SpineMMCls_*"))
    #checkpoints = list(Path("/home/nash/PycharmProjects/PfirrmannGrading/multirun/2022-06-28/16-32-33/6/results/models").glob("SpineMMCls_*"))
